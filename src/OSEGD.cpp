...
void OSEGD::Impl::write_general_header1()
{
    ...
    common.general_header.file_number > 9999 ? write_u16(&ptr, 0xffff) :
	   	to_bcd(&ptr, common.general_header.file_number, false, 4);
    ...
    *ptr++ = common.general_header.base_scan_int * pow(2, 4);
    ...
    int exp = static_cast<unsigned>(log2(common.general_header.
    ...
    *ptr++ = common.general_header.scans_per_block / pow(2, exp);
    ...
    common.general_header.extended_hdr_blocks > 99 ?
	   	(void)(*ptr++ = static_cast<uint8_t>(0xff)) :
	   	to_bcd(&ptr, common.general_header.extended_hdr_blocks, false, 2);
    common.general_header.external_hdr_blocks > 99 ?
	   	(void)(*ptr++ = static_cast<uint8_t>(0xff)) :
	   	to_bcd(&ptr, common.general_header.external_hdr_blocks, false, 2);
...
void OSEGD::Impl::write_ch_set_hdr(CommonSEGD::ChannelSetHeader& hdr)
{
    ...
    if (!common.general_header.add_gen_hdr_blocks ||
        ...
        uint16_t val = static_cast<uint16_t>(hdr.descale_multiplier *
											 pow(2, 10)) & 0x7fff;
    ...
    common.file.write(common.ch_set_hdr_buf.data(),
...
void OSEGD::Impl::assign_sample_writers()
{
        ...
        write_sample = [this](char** buf, double val) {
            ...
            if (counter == 4) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    exp[i] = static_cast<uint16_t>(log(abs_val) /
												   log(2) + 1 + 15) & 0xf;
    ...
    case 8022:
        ...
        write_sample = [](char** buf, double val) {
            ...
            uint32_t exp = static_cast<uint32_t>(log(abs_val) /
												 log(2) + 1 + 4) & 0b111;
            uint32_t frac = static_cast<uint32_t>(abs_val /
												  pow(2, exp - 4)) & 0xf;
    ...
    case 8042:
        ...
        write_sample = [](char** buf, double val) {
            ...
            uint32_t exp = static_cast<uint32_t>(log(abs_val) /
												 log(2) / 4 + 1) & 0b11;
            uint32_t frac = static_cast<uint32_t>(abs_val / pow(16, exp) *
												  pow(2, 5)) & 0x1f;
    ...
    case 8048:
        ...
        write_sample = [this](char** buf, double val) {
            uint32_t sign = val < 0 ? 1 : 0;
            double abs_val = abs(val);
            uint32_t exp = static_cast<uint32_t>(log(abs_val) / log(2) / 4 + 1)
			   	& 0b01111111;
            uint32_t frac = static_cast<uint32_t>(abs_val / pow(16, exp) *
												  pow(2, 24)) & 0xffffff;
    ...
    case 9036:
        ...
        if (endian::native == endian::big)
            write_sample = [this](char** buf, double val)
		   	{ return write_i24(buf, swap(static_cast<int32_t>(val))); };
        else
            write_sample = [](char** buf, double val) {
                write<int16_t>(buf, val);
                write<int8_t>(buf, static_cast<int32_t>(val) >> 16);
    ...
    case 9038:
        ...
        if (endian::native == endian::big)
            write_sample = [this](char** buf, double val)
		   	{ return write_i32(buf, swap(static_cast<int32_t>(val))); };
        else
            write_sample = [](char** buf, double val)
		   	{ return write<int32_t>(buf, val); };
        break;
    case 9058:
        common.bits_per_sample = 32;
        if (endian::native == endian::big)
...
void OSEGD::write_general_header2_rev2()
{ pimpl->write_general_header2_rev2(); }
void OSEGD::write_rev2_add_gen_hdrs() { pimpl->write_rev2_add_gen_hdrs(); }
void OSEGD::write_general_header2_and_3()
{ pimpl->write_general_header2_and_3(); }
void OSEGD::write_rev3_add_gen_hdrs() { pimpl->write_rev3_add_gen_hdrs(); }
void OSEGD::write_ch_set_hdr(CommonSEGD::ChannelSetHeader& hdr)
